...
| **$t_{b}$** | 空白明け時間 | アクションによるロックが解ける秒数（$t_{min} \le t_{b} \le t_{max}$ の場合のみ適用） |
...
※変形後の式を用いた場合： $E[T] = \frac{15 + 5}{2} + \frac{(10 - 5)^2}{2(15 - 5)} = 10 + \frac{25}{20} = 11.25$
...
## 5. 結論
本計算モデルを使用することで、ルアーアクション後の空白時間によって平均待機時間が単純な中央値からズレる場合でも、より正確にシミュレートすることが可能です。
